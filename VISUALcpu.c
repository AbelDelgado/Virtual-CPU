...
#define	RdMask	0x000fb	// mask for destination register
...
#define Imediate 		((INSTRUCTREG & imvMask) >> 0x4)  
#define Imediate2 		((INSTRUCTREG2 & imvMask) >> 0x4)  
...
/******************************************************************
	...
	int getOffset(int * finalAddress)
	{
		...
		sscanf_s(inputAddress, "%X", finalAddress);
	}
	...
	int getLength(int * finalLengthOfMessage)
	{
		...
		sscanf_s(inputLength, "%X", finalLengthOfMessage);
	}
