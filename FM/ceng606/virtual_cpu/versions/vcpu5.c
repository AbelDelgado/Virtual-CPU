...
#define  brOffset8	((PC & 0x000000ff) + offset8b + 0x00fe)
#define	_8bBrAddr	((PC & 0xffffff00) | (brOffset8 & 0x000000ff))
#define  brOffset12	((PC & 0x00000fff) + offset12b + 0x0ffe)
#define	_12bBrAddr	((PC & 0xfffff000) | (brOffset12 & 0x00000fff))
...
#define	RnLSBb	(Rn & 0x000000ff)				 // least significant byte
#define	imValue	((IR & 0x0ff0) >> 4u)
#define	byteMsk	(0x000000ff)
#define	_8bits	8u
#define	brCOND	((IR & 0x0f00u) >> 8u)
#define	offset8b	(IR & 0x00ffu)
...
void condiBr (void)
{
	 switch (brCOND)
	 {
		  case EQ: if (Z == 1) setPC(_8bBrAddr);
							break;
		  case NE: if (Z == 0) setPC(_8bBrAddr);
							break;
		  case CS: if (C == 1) setPC(_8bBrAddr);
							break;
		  case CC: if (C == 0) setPC(_8bBrAddr);
							break;
		  case MI: if (N == 1) setPC(_8bBrAddr);
							break;
		  case PL: if (N == 0) setPC(_8bBrAddr);
							break;
		  case HI: if ((C == 1) && (Z == 0)) setPC(_8bBrAddr);
							break;
		  case LS: if ((C == 0) && (Z == 1)) setPC(_8bBrAddr);
							break;
		  case AL: setPC(_8bBrAddr);
					  break;
	 }
}
...
/*
 * loadRd () - loads a byte or word from address Rn in main memory to Rd
 */
void loadRd (void)
{
	 int bytecnt;
...
/*
 * storeRd () - store Rd to main memory address held by Rn
 */
void storeRd (void)
{
	 signed int bytecnt;
	 MAR = Rn;
	 MBR = Rd;
	 if (B == 1) 	// store lsbyte of Rd 
	 {
		  MEM = (unsigned char) MBR;
		  return;
	 }
	 for (bytecnt = word - 1 ; bytecnt >= 0; bytecnt--) // store word, bytes 3 - 0
	 {
