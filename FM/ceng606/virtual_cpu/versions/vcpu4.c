...
void loadMem (unsigned long memAddr,  unsigned long * reg, unsigned short byteflag);
void shiftRd (void);
...
void trace (void);
void writeMem (unsigned long dstAddr, unsigned long * reg, unsigned short byteflag);
...
void execute (void)
{
	 switch (Itype)
	 {
		  ...
		  case cndBr: condiBr();
						  break;
		  case BRL: PC = (unsigned) offset12b;
						break;
		  ...
		  case LdStr: switch (L)
						  {
								case 0: writeMem(Rn, &Rd, B);
										  break; 
								case 1: loadMem(Rn, &Rd, B);
										  break; 
						  }
						  break;
	 }
...
void writeMem (unsigned long memAddr, unsigned long * reg, unsigned short byteflag)
{
